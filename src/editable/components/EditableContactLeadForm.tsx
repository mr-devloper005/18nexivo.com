...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 rounded-3xl bg-white p-6 shadow-sm">
      ...
      <button type="submit" disabled={status === 'submitting'} className="mt-6 inline-flex h-[3.5rem] w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent)] px-6 text-xs font-black uppercase tracking-[0.12em] text-white transition hover:bg-[#001316] disabled:cursor-not-allowed disabled:opacity-70">
...
function Field({ name, label, type = 'text', placeholder, required = false }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="grid gap-2 text-sm font-black opacity-75">
      {label}
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-[3.5rem] rounded-2xl border border-black/10 bg-white px-4 text-base font-medium outline-none transition focus:border-[var(--slot4-accent)]" />
    </label>
  )
}
