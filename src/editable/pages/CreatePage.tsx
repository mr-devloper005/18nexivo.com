...
const fieldClass = 'rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-bold text-[#06181b] outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-accent)]'
    ...
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[#f4f4f2] px-4 py-12 text-[#06181b] sm:px-6 lg:px-8 lg:py-16">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-3xl bg-white p-7 shadow-[0_22px_60px_rgba(0,19,22,.08)] md:grid-cols-[.75fr_1.25fr] md:p-9">
            <div className="flex h-full min-h-64 items-center justify-center rounded-3xl bg-[#001316] text-white">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-full bg-[#eef0ef] px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
        ...
        <section className="mx-auto max-w-6xl">
          <div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr]">
            <aside className="rounded-3xl bg-[#001316] p-7 text-white sm:p-9">
              ...
              <p className="mt-5 max-w-xl text-sm font-semibold leading-8 text-white/70">{pagesContent.create.hero.description}</p>
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  const Icon = taskIcon[item.key] || FileText
                  const active = item.key === task
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-[var(--slot4-accent)] bg-[var(--slot4-accent)] text-white' : 'border-white/15 bg-white/10 text-white hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-3xl bg-white p-6 shadow-[0_22px_60px_rgba(0,19,22,.08)] sm:p-8">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-[#eef0ef] px-4 py-2 text-xs font-black uppercase tracking-[0.12em]">{session.name}</span>
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-black uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5 hover:bg-[#001316]">
