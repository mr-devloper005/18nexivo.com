...
const uniquePosts = (posts: SitePost[]) =>
  Array.from(new Map(posts.filter((post) => post.slug).map((post) => [post.slug || post.id || post.title, post])).values())
...
const isRealMediaDistributionPost = (post: SitePost) => {
  ...
  const explicitType = asText(content.type).toLowerCase()
  if (explicitType === 'comment') return false
  const taskKey = getPostTaskKey(post)
  return taskKey === 'mediaDistribution' || !taskKey
}

async function fetchEditableMediaDistributionPosts(page: number, category: string) {
  const limit = 24
  const feeds = await Promise.all([
    fetchSiteFeed(1000, { fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(1000, { task: 'mediaDistribution', fresh: true, timeoutMs: 5000 }).catch(() => null),
  ])
  const allPosts = uniquePosts(feeds.flatMap((feed) => feed?.posts || [])).filter(isRealMediaDistributionPost)
  const filteredPosts = category === 'all'
    ? allPosts
    : allPosts.filter((post) => normalizeCategory(getCategory(post, '')) === category)
  const start = (page - 1) * limit
  const posts = filteredPosts.slice(start, start + limit)
  return { posts, pagination: fallbackPagination(page, limit, filteredPosts.length) }
}
...
export async function EditableTaskArchiveRoute({
  ...
  const { posts, pagination } = task === 'mediaDistribution'
    ? await fetchEditableMediaDistributionPosts(page, category)
    : await fetchPaginatedTaskPosts(task, { page, limit: 24, category })
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const dynamicCategories = Array.from(new Map([
    ...CATEGORY_OPTIONS,
    ...(task === 'mediaDistribution' ? mediaDistributionCategories.defaults : []),
    ...posts.map((post) => {
...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f4f4f2] text-[#06181b]">
        <section className="relative overflow-hidden bg-[#001316] text-white">
          <div className="absolute inset-0 opacity-30 [background:linear-gradient(166deg,transparent_0_58%,#ff604a_58%_63%,transparent_63%),radial-gradient(circle_at_78%_36%,#ff604a_0,transparent_22%)]" />
          <div className="relative mx-auto flex max-w-[1296px] flex-col gap-6 px-4 py-16 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-24">
            <div>
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-white/70">
              Real media-distribution posts, campaign updates, releases, and partner announcements organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="bg-white shadow-[0_12px_30px_rgba(0,19,22,.08)]">
          <div className="mx-auto flex max-w-[1296px] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.12em] sm:px-6 lg:px-8">
        ...
        {lead ? (
          <section className="mx-auto mt-12 grid max-w-[1296px] overflow-hidden rounded-3xl bg-white shadow-[0_25px_70px_rgba(0,19,22,.1)] lg:grid-cols-[1.6fr_.8fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[30rem] overflow-hidden">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-[0.98] sm:text-5xl">{lead.title}</h2>

              </div>
            </Link>
            <div className="grid">
              <div className="bg-[var(--slot4-accent)] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.18em]">Top dispatches</p>
                <p className="mt-3 text-3xl font-black leading-tight">What the distribution desk is routing now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-t border-black/10 bg-white">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[1296px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-[#06181b] pb-4">
            <h2 className="text-4xl font-black sm:text-5xl">All media updates</h2>
            <form action={basePath} className="flex rounded-full bg-white shadow-sm">
              ...
              <button className="h-11 rounded-full bg-[#001316] px-5 text-xs font-black uppercase tracking-[0.08em] text-white">Filter</button>
            </form>
          </div>

          {remaining.length ? (
            <div className="logi-stagger grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-[0_22px_55px_rgba(0,19,22,.12)]">
                  <div className="aspect-[16/10] overflow-hidden bg-[#001316]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--slot4-accent)]">
